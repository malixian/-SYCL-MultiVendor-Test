...
int main(int argc, char* argv[])
{
  ...
  printf("Average execution time of ReluGrad_impl1 Kernel: %f (ms)\n",
          (time * 1e-6f) / repeat);
  ...
  printf("Average execution time of ReluGrad_impl2 Kernel: %f (ms)\n",
          (time * 1e-6f) / repeat);
  ...
  time = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
  printf("Average execution time of Relu_impl1 Kernel : %f (ms)\n",
          (time * 1e-6f) / repeat);
  ...
  time = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
  printf("Average execution time of Relu_impl2 Kernel: %f (ms)\n",
          (time * 1e-6f) / repeat);
