    ...
    unsigned int *origOffsets)
{
  ...
  cudaMemcpy(d_input_buff, d_input, sizeof(float)*(listsize + DIVISIONS*4), cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(d_offsets_buff, h_offsets, sizeof(unsigned int)*DIVISIONS, cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(d_pivotPoints_buff, pivotPoints, sizeof(float)*DIVISIONS, cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(d_bucketOutput, d_output, sizeof(float)*(listsize + DIVISIONS*4), cudaMemcpyHostToDevice); 
