...
int particleFilter(unsigned char * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles) {
  ...
  for (k = 1; k < Nfr; k++) {
    ...
    q.submit([&](sycl::handler& cgh) {
      sycl::local_accessor<float, 1> u1(1, cgh);
      sycl::local_accessor<float, 1> sumWeights(1, cgh);
      //accessor <float, 1, read_write, local> u1(1, cgh);
      //accessor <float, 1, read_write, local> sumWeights(1, cgh);
